...
describe('greet handler', () => {
    ...
    test('falls back to the default prefix with no secrets configured', async () => {
        const out = await handler({ name: 'Ada' }, {})
    ...
    test('uses the declared GREETING_PREFIX variable', async () => {
        const out = await handler({ name: 'Ada' }, { secrets: { GREETING_PREFIX: 'Howdy' } })

        expect(out.content[0].text).toBe('Howdy, Ada!')
    })

    test('ignores unrelated keys in extra.secrets', async () => {
        const out = await handler({ name: 'Ada' }, { secrets: { LLMA_ANALYTICS_KEY: 'leak-me-not' } })
